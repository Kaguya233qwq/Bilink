...
class Login:
    ...
    async def get_qrcode() -> Optional[Dict[str, str]]:
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.get(
                url=Api.QRCODE_GENERATE, headers=create_headers(), follow_redirects=True
            )
    ...
    async def save_qrcode(url_qrcode) -> None:
        ...
        qr_code = qrcode.QRCode(
            error_correction=qrcode.constants.ERROR_CORRECT_L, box_size=1, border=2
        )
...
async def login_by_qrcode() -> Optional[Dict[str, str]]:
    ...
    await Login.save_qrcode(qr.get("url"))  # 保存二维码图片并输出至终端
    Logger.success("二维码生成成功，请使用bilibili客户端扫描确认")
    return await Login.polling(qr.get("key"))
